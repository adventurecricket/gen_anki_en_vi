...
class Word(object):
    ...
    def definition_full(cls):
        ...
        try:
            ...
            def_body_tags = cls.soup_data.select(cls.definition_body_mul_selector)
        except IndexError:
            pass
        
        info = []
        if (not namespace_tags) and (not def_body_tags):
            # Word has none space
            info = cls.definition_full_without_space(cls.definition_body_sgl_selector)
        else:
            info = cls.definition_mul_full(namespace_tags)
    ...
    @classmethod
    def extra_info(cls):
        """ get extra information on top """
        if cls.soup_data is None:
            return None
        try:
            return cls.soup_data.select(cls.extra_info_selector)[0].text
        except IndexError:
            pass

    @classmethod
    def definition_mul_full(cls, namespace_tags):
        """ return word definition + corresponding examples for word with multiple sense
        ...
        if len(info) == 0:
            info = cls.definition_full_without_space(cls.definition_body_mul_selector)

        return info

    @classmethod
    def definition_full_without_space(cls, definition_body_selector):
        """ return word definition + corresponding examples for word with single sense
